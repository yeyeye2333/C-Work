...
static const int STUDIO_PAGE_SIZE = 5; // 每页显示演出厅的个数
...
void Studio_UI_MgtEntry(void) {
    ...
    paging.totalRecords = Studio_Srv_FetchAll(head); // 获取演出厅的总数
    ...
    do {
        ...
        Paging_ViewPage_ForEach(head, paging, studio_node_t, pos, i) {
            printf("%5d  %18s  %10d  %10d  %10d\n", pos->data.id, pos->data.name, pos->data.rowsCount, pos->data.colsCount, pos->data.seatCount);
        }
...
int Studio_UI_Add(void) {
    ...
    do {
        ...
        printf("Room Name:");
        fflush(stdin);
        fgets(rec.name, 10, stdin);
        rec.name[strlen(rec.name) - 1] = '\0'; // 删除名称后面的换行符
