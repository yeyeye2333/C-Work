...
// 向上翻页
#define PREV_PAGE(list, paging, node_t) {                           \
            if (!Pageing_IsFirstPage(paging)) {                     \
                Paging_Locate_OffsetPage(list, paging, -1, node_t); \
            }                                                       \
        }

// 向下翻页
#define NEXT_PAGE(list, paging, node_t) {                           \
...
// 简化malloc函数
#define new(type_t) (type_t *) malloc(sizeof(type_t))
...
// 释放链表list中所有数据结点。list为链表头指针，list_node_t为链表结点类型
